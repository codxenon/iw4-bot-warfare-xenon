...
spawnIntermission()
{
	...
	if ( level.rankedMatch && ( self.postGamePromotion || self.pers["postGameChallenges"] ) )
	{
		...
		//self openMenu( game["menu_endgameupdate"] ); -- removed to allow mapvoting buttons and view
		...
		//self closeMenu( game["menu_endgameupdate"] ); -- removed to allow mapvoting buttons and view
	}
	...
	if(isDefined(level.mapvote_active) && level.mapvote_active)
		self thread scripts\mp\bots_mapvote::mapvote_cinematic();
}
